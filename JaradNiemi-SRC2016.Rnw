...
The predictive distribution for $\tilde{y}$ conditional on $\hat{\theta}$ and $\tilde{x}$ \pause is Student $t$ with $N$ degrees of freedom, mean 
\[ 
...
\[
D_n(\tilde{x}) = \{ (x_i,y_i): K_\theta(\tilde{x},x_i) \le \delta_{(n)} \}
\]
...
\[ 
\mbox{argmin}_{x_{j+1} \in D_{N'}\setminus D_j} v_j(x_{j+1},D_j,\tilde{x})
\]
\pause
...
for the ALC calculation at each of $n_{\mathrm{cand}}=60000$ candidate
...
{\footnotesize
Comparing full global approximation times for a $\sim40$K-sized
design, and a $\sim 10$K predictive locations.
}
\end{frame}
